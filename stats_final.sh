...
PY_CODE=$(echo    "$CLOC_CORE" | grep "^Python "   | awk '{print $5}')
TOML_CODE=$(echo  "$CLOC_CORE" | grep "^TOML "     | awk '{print $5}')
MD_CODE=$(echo    "$CLOC_CORE" | grep "^Markdown "  | awk '{print $5}')
SH_CODE=$(echo    "$CLOC_CORE" | grep "^Bourne Shell\|^Bash " | awk '{sum+=$NF} END{print sum+0}')
...
TEST_LINES=$(count_lines      "tests/test_aicli.py")
...
# Raw count
echo -e "${YELLOW}Method 1: Raw line count (all files, all languages)${NC}"
...
printf "  %-40s  %5s lines\n"  "aicli/app.py (CLI entry)"          "$APP_LINES"
...
printf "  %-40s  %5s lines\n"  "tests/test_aicli.py"               "$TEST_LINES"
echo ""
...
echo -e "  ${DIM}(Plus $(format_num $((TOML_CODE + SH_CODE + MD_CODE))) lines of config/scripts/docs)${NC}"
echo ""
...
echo -e "    • Self-correction loop on shell errors"
...
echo -e "    • Session export to Markdown or JSON (aicli export)"
echo -e "    • Autonomous agent mode with plan/execute/feedback loop (aicli agent)"
echo -e "    • Multimodal vision: --image flag for PNG/JPEG/GIF/WebP (F2)"
echo ""
...
TEST_COUNT_TOTAL=$((TEST_COUNT_UNIT + TEST_COUNT_INTEG))
echo -e "  Unit tests:        ${GREEN}${BOLD}${TEST_COUNT_UNIT}${NC}  (tests/test_aicli.py — mocks/patches, no live API)"
echo -e "  Integration tests: ${GREEN}${BOLD}${TEST_COUNT_INTEG}${NC}  (tests/test_integration.py — component wiring, still mocked)"
echo -e "  Total:             ${GREEN}${BOLD}${TEST_COUNT_TOTAL}/${TEST_COUNT_TOTAL} passing${NC}"
echo ""
