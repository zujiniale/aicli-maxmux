...
echo "                                 provider, code_runner, serve, mcp_server"
echo "    shell_integration.zsh     ← Ctrl+G hotkey for zsh"
echo "    shell_integration.bash    ← Ctrl+G hotkey for bash"
...
echo "  tests/test_tui_pure.py      ← TUI pure logic + vim nav tests"
echo "  tests/test_graph_server.py  ← graph server + node tags tests"
echo "  tests/test_serve.py         ← HTTP API server tests"
echo "  tests/test_web_search.py    ← web search chain tests"
echo "  tests/test_new_commands.py  ← v1.5.3/v1.5.4 CLI additions tests"
echo "  tests/test_mcp_server.py    ← MCP server tests (60 tests)"
...
echo "After expanding, remember to re-run:"
echo "   10. Re-install hotkey:   aicli config install-shell"
echo ""
